...
module.exports = class MusicServer {
  ...
  _handler(method) {
    ...
    switch (true) {
      ...
      case /(?:^|\/)audio\/cfg\/getmonitorstatus(?:\/|$)/.test(url):
        // Dummy response to not gett error all the time
        return this._emptyCommand(url, [{ scanning: 0 }]);
  ...
  async _audioSync(url) {
    ...
    const zone = this._zones[+zoneId - 1];
    const zonesToSync = parts.slice(3);

    for (const zoneId of zonesToSync) {
      if (zoneId !== '') {  // Ignore empty strings resulting from trailing slashes
